...
trains an unsupervised reconstruction model on benign classes,
exports artifacts (model, scaler, threshold, ONNX optional),
AND generates accurate evaluation plots on the Test Set only.
"""
...
import matplotlib
# Force Agg backend to run on headless servers (Jetson/Cloud)
matplotlib.use("Agg")
...
def generate_evaluation_plots(
    ...
    """Generates and saves Histogram, ROC, and PR curves based on Test Set only."""
    output_dir.mkdir(parents=True, exist_ok=True)

    # Separate benign vs trojan based on y_true (0=Benign, 1=Trojan)
    benign_errors = test_errors[y_true == 0]
    trojan_errors = test_errors[y_true == 1]
    ...
    if len(trojan_errors) > 0:
        plt.hist(trojan_errors, bins=bins, alpha=0.6, density=True, label=f"Trojan ({len(trojan_errors)})")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / "reconstruction_error_distribution.png", dpi=160)
    plt.close()
    ...
    if len(np.unique(y_true)) > 1:
        ...
        precision, recall, _ = precision_recall_curve(y_true, test_errors)
        ...
        print("Skipping ROC/PR plots: Test set contains only one class.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--window-stride", type=int, default=5, help="Stride between windows")
    parser.add_argument("--train-labels", nargs="*", default=["idle", "normal"], help="Labels treated as benign")
    parser.add_argument(
        "--benign-eval-labels",
        nargs="*",
        default=None,
        help="Which benign labels to include in the test set evaluation",
    )
...
def main() -> None:
    ...
    # Logic for ground truth: 0 = Benign, 1 = Trojan
    if args.anomaly_mode == "trojan":
        y_true = np.array(
    ...
    try:
        auc_score = float(roc_auc_score(y_true, test_errors)) if len(np.unique(y_true)) > 1 else math.nan
    except Exception:
        auc_score = math.nan
    ...
    # --- Generate Plots Immediately (Using exactly the same data as metrics) ---
    print("Generating evaluation plots on Test Set...")
    ...
    print(f"Artifacts and plots written to {args.output_dir}")
